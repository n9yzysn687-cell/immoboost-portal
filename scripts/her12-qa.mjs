...
const v11=fs.readFileSync(path.join(her12,'experience-v11.js'),'utf8');for(const token of ['guideSets','startTimer=function','Puis série','allDone','her11-resting','timerSkip'])if(!v11.includes(token))fail(`v11 guided set flow missing expected primitive: ${token}`);
...
for(const ref of ['./enhancements-v3.css','./enhancements-v3.js','./experience-v4.css','./experience-v4b.js','./experience-v5.css','./experience-v5.js','./experience-v6.css','./experience-v6.js','./experience-v7.css','./experience-v7.js','./experience-v8.css','./experience-v8.js','./experience-v9.css','./experience-v9.js','./experience-v10.css','./experience-v10.js','./experience-v11.css','./experience-v11.js','WARM_MEDIA','her12-v14'])if(!sw.includes(ref))fail(`service worker missing current cache primitive: ${ref}`);

if(!process.exitCode)console.log(`HER12 QA OK: ${sessions.length} sessions, ${ids.size} unique exercises, ${media.size} local images, glute planning ${gluteVolume.map(v=>v.toFixed(2)).join(' -> ')} effective-set estimate, triceps ${tricepsVolume.join(' -> ')}, loaded abs ${abs.sets.join(' -> ')}, goal progress + guided set/rest flow verified.`);
