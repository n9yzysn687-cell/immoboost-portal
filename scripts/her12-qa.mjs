...
const v10=fs.readFileSync(path.join(her12,'experience-v10.js'),'utf8');for(const token of ['homeVisual','v10SessionVisual','v10WeekStrip','exerciseBadge','her10-ready'])if(!v10.includes(token))fail(`v10 visual balance missing expected primitive: ${token}`);
for(const [name,source] of [['runtime-v2.js',runtime],['enhancements-v3.js',enhancements],['experience-v4b.js',v4],['experience-v5.js',v5],['experience-v6.js',v6],['experience-v7.js',v7],['experience-v8.js',v8],['experience-v9.js',v9],['experience-v10.js',v10]]){try{new vm.Script(source)}catch(error){fail(`${name} syntax error: ${error.message}`)}}
...
for(const ref of ['./enhancements-v3.css','./enhancements-v3.js','./experience-v4.css','./experience-v4b.js','./experience-v5.css','./experience-v5.js','./experience-v6.css','./experience-v6.js','./experience-v7.css','./experience-v7.js','./experience-v8.css','./experience-v8.js','./experience-v9.css','./experience-v9.js','./experience-v10.css','./experience-v10.js','WARM_MEDIA','her12-v13'])if(!sw.includes(ref))fail(`service worker missing current cache primitive: ${ref}`);

if(!process.exitCode)console.log(`HER12 QA OK: ${sessions.length} sessions, ${ids.size} unique exercises, ${media.size} local images, glute planning ${gluteVolume.map(v=>v.toFixed(2)).join(' -> ')} effective-set estimate, triceps ${tricepsVolume.join(' -> ')}, loaded abs ${abs.sets.join(' -> ')}, v10 balanced visual flow verified.`);
