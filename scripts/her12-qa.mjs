...
const mustExist=['index.html','styles-v2.css','enhancements-v3.css','experience-v4.css','experience-v5.css','experience-v6.css','experience-v7.css','experience-v8.css','experience-v9.css','experience-v10.css','experience-v11.css','program-v2.js','runtime-v2.js','enhancements-v3.js','experience-v4b.js','experience-v5.js','experience-v6.js','experience-v7.js','experience-v8.js','experience-v9.js','experience-v10.js','experience-v11.js','manifest.webmanifest','sw.js','icon.svg','apple-touch-icon.png'];
...
const gluteVolume=[0,1,2].map(phase=>Object.values(program).reduce((sum,session)=>sum+session.ex.reduce((s,ex)=>s+(gluteWeights[ex.id]||0)*(ex.sets?.[phase]||0),0),0));
...
const tricepsVolume=[0,1,2].map(phase=>(tri?.sets?.[phase]||0)+(ohtri?.sets?.[phase]||0));
...
if((abs.sets||[]).some(v=>v<3||v>4))fail(`loaded abs sets should stay compact at 3-4 sets; got ${abs.sets?.join('/')}`);
...
const v11=fs.readFileSync(path.join(her12,'experience-v11.js'),'utf8');for(const token of ['guideSets','startTimer=function','Puis série','allDone','her11-resting','timerSkip','REST_PROFILE','Repos recommandé','hip:150','press:150','tri:90'])if(!v11.includes(token))fail(`guided rest flow missing expected primitive: ${token}`);
for(const [name,source] of [['runtime-v2.js',runtime],['enhancements-v3.js',enhancements],['experience-v4b.js',v4],['experience-v5.js',v5],['experience-v6.js',v6],['experience-v7.js',v7],['experience-v8.js',v8],['experience-v9.js',v9],['experience-v10.js',v10],['experience-v11.js',v11]]){try{new vm.Script(source)}catch(error){fail(`${name} syntax error: ${error.message}`)}}
...
for(const ref of ['./enhancements-v3.css','./enhancements-v3.js','./experience-v4.css','./experience-v4b.js','./experience-v5.css','./experience-v5.js','./experience-v6.css','./experience-v6.js','./experience-v7.css','./experience-v7.js','./experience-v8.css','./experience-v8.js','./experience-v9.css','./experience-v9.js','./experience-v10.css','./experience-v10.js','./experience-v11.css','./experience-v11.js','WARM_MEDIA','her12-v15'])if(!sw.includes(ref))fail(`service worker missing current cache primitive: ${ref}`);

if(!process.exitCode)console.log(`HER12 QA OK: ${sessions.length} sessions, ${ids.size} unique exercises, ${media.size} local images, glute planning ${gluteVolume.map(v=>v.toFixed(2)).join(' -> ')} effective-set estimate, triceps ${tricepsVolume.join(' -> ')}, loaded abs ${abs.sets.join(' -> ')}, goal progress + research-tuned set/rest flow verified.`);
