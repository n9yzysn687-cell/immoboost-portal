const CACHE="her12-v6";
const SHELL=[
  "./","./index.html","./styles-v2.css","./enhancements-v3.css","./program-v2.js","./runtime-v2.js","./enhancements-v3.js","./manifest.webmanifest","./icon.svg","./apple-touch-icon.png"
];
const MEDIA=[
...
self.addEventListener("message",event=>{
  ...
  event.waitUntil(caches.open(CACHE).then(async cache=>{
    const existing=await Promise.all(MEDIA.map(file=>cache.match(file)));
    const missing=MEDIA.filter((_,i)=>!existing[i]);
    await Promise.allSettled(missing.map(file=>cache.add(file)));
  }));
});

self.addEventListener("fetch",event=>{
  if(event.request.method!=="GET")return;
  const url=new URL(event.request.url),isLocal=url.origin===self.location.origin,isMedia=isLocal&&url.pathname.includes("/her12/assets/");

  if(event.request.mode==="navigate"&&isLocal){
